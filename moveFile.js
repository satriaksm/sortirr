...
/**
 * Move a file from source directory (dump or custom folder) to target category folder safely.
 * If a file with the same name already exists at destination, generates a unique non-conflicting name.
 */
function moveFile(fileName, folder, customDataDir, customSourceDir) {
    const dataDir = customDataDir || process.env.SORTIRR_DATA_DIR;

    const sourcePath = customSourceDir
        ? path.join(customSourceDir, fileName)
        : (dataDir 
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(sourcePath)) {
            return reject(new Error(`Source file "${fileName}" not found in source directory (${path.dirname(sourcePath)})`));
        }
